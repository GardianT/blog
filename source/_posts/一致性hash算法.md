...
- 查找比较简单，就是计算hash值，寻找虚拟节点vector的bound。如果bound到了边界，返回边界的节点（形成环）。这样解决了宕机掉节点问题：如果一个机器宕机，当hash到与这个机器对应的range的时候，由于这个虚拟节点（对应的真实server）已经宕机，所以他会继续寻找下一个虚拟节点。而由于虚拟节点是随机插入的，所以也能保证server宕机的时候，他负责的range会被随机的其他若干server承担。
